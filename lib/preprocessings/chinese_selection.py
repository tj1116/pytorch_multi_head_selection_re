...
class Chinese_selection_preprocessing(object):
    ...
    def gen_bio_vocab(self):
        ...
        #result = {'<pad>': 5, 'B': 0, 'I': 1, 'O': 2, '<start>': 2, '<stop>': 4}
        json.dump(result,
    ...
    def gen_ERNIE_vocab(self):
        source = os.path.join(self.hyper.ERNIE_pretrain_path, 'vocab.txt')
        target = os.path.join(self.data_root, 'word_ERNIE_vocab.json')

        ERNIE_vocab = []
        with open(source, 'r') as s:
            for l in s:
                ERNIE_vocab.append(l.strip("\n"))

        result = {j:i for i, j in enumerate(ERNIE_vocab)}
        json.dump(result, open(target, 'w'), ensure_ascii=False)
    ...
    def spo_to_selection(self, text: str, spo_list: List[Dict[str, str]]
        ...
        for triplet in spo_list:
            ...
            '''
            object_pos = text.find(object) + len(object)
            relation_pos = self.relation_vocab[triplet['predicate']]
            subject_pos = text.find(subject) + len(subject)
...
'''
class Chinese_ERNIE_selection_preprocessing(object):
    def __init__(self, hyper):
        self.hyper = hyper
        self.raw_data_root = hyper.raw_data_root
        self.data_root = hyper.data_root
    ...
    def gen_bio_vocab(self):
        ...
        json.dump(result,
                  open(os.path.join(self.data_root, 'bio_ERNIE_vocab.json'), 'w'))
...
if __name__=='__main__':
    ...
    hyper = Hyper(os.path.join('experiments',
                                    'chinese_selection_re' + '.json'))

    test = Chinese_ERNIE_selection_preprocessing(hyper)
'''